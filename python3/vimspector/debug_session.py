...
class DebugSession( object ):
    ...
    def Start( self,
        ...
        while 'extends' in configuration:
            ...
            base_configuration = all_configurations.get(
        ...
        while 'extends' in adapter:
            ...
            if base_adapter is None:
                suggested_gadgets = installer.FindGadgetForAdapter(
                    base_adapter_name )
                if suggested_gadgets:
                    response = utils.AskForInput(
    ...
    def _StartWithConfiguration( self, configuration, adapter ):
        def start():
            ...
            if not self._StartDebugAdapter():
                self._logger.info(
                    "Failed to launch or attach to the debug adapter" )
                return
    ...
    def Destroy( self ):
        ...
        if self._connection is not None:
            raise RuntimeError(
                "Can't destroy a session with a live connection" )
    ...
    def ReadSessionFile( self, session_file: str = None ):
        if session_file is None:
            session_file = self._DetectSessionFile(
                invent_one_if_not_found = False )
    ...
    def WriteSessionFile( self, session_file: str = None ):
        ...
        try:
            ...
            self._logger.exception(
                f"Unable to write session file { session_file }" )
            utils.UserMessage( "The session file could not be read",
                               persist = True,
                               error = True )
            return False
    ...
    def ShowDisassembly( self ):
        ...
        with utils.LetCurrentWindow( self._codeView._window ):
            vim.command(
                f'rightbelow { settings.Int( "disassembly_height" ) }new' )
    ...
    def _Initialise( self ):
        ...
        def handle_initialize_response( msg ):
            ...
            # representation out?
            if not self.parent_session:
                self._breakpoints.SetServerCapabilities(
                    self._server_capabilities )
            self._Launch()
    ...
    def _Prepare( self ):
        ...
        if self._launch_config is None:
            ...
            self._launch_config.update(
                self._adapter.get( 'configuration', {} ) )
    ...
    def OnEvent_breakpoint( self, message ):
        ...
        if reason == 'changed':
            ...
            utils.UserMessage(
              'Unrecognised breakpoint event (undocumented): {0}'.format(
                  reason ),
              persist = True )
