...
* `VimspectorLoadSession` - read `[.vim/].vimspector.session`
...
path when calling the command. When loading a session, the `file name` is found
by recursively searching up the directory hierarchy from the directory of the
file open in Vim. For each recursion, the path is also checked relative to the
`.vim` subdirectory.
