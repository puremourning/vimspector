...
session. Unfortunately Vimspector is not a task manager and implementing this functionality
is out of the scope of this project. However, there are some strategies to achieve
similar functionality.

To use Vim's built-in make command (`:h make`) to rebuild and run the debugger afterwards you can
...
Since this solution requires you to press enter every time you want to debug,
it is not optimal.  Here is one proposal for a more convenient solution:

Install the [vim-dispatch plugin](https://github.com/tpope/vim-dispatch), which provides
an asynchronous `:Make` alternative to `:make`. Since `:Make` is asynchronous, 
its successful completion has to be checked differently using `autocmd QuickFixCmdPost make`.

Now you can create a project-specific configuraton using an `.exrc` file (see `:h exrc`)
in you project root and add it to your Git repository.

An example would be:

```Vim
" Create a custom debug commmand
command! Debug call BuildAndDebug()

" Create a function that builds, closes the quickfix window and runs Vimspector
...
" Map this command to F12
nmap <F12> :Debug<CR>

```

Now you can just press F12 to rebuild and debug.
